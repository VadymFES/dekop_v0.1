...
export default function ProductForm({ product }: ProductFormProps) {
  ...
  const generateSlug = async () => {
    ...
    try {
      ...
      const response = await fetch(`${adminPath}/api/products/check-slug?slug=${encodeURIComponent(baseSlug)}${excludeParam}`);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const url = isEdit ? `${adminPath}/api/products/${product!.id}` : `${adminPath}/api/products`;
  ...
  const handleSaveProductImages = async () => {
    ...
    try {
      ...
      const response = await fetch(`${adminPath}/api/products/${product.id}/images`, {
        method: 'PUT',
        headers,
        body: JSON.stringify({
          type: 'images',
